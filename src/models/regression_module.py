...
class ProteinLitModule(LightningModule):
    """LightningModule for multi-modal protein **regression** (stability/fluorescence).
    Fuses ESM-C+Ankh and ProtT5+PGLM, then predicts a single scalar target."""
    def __init__(
        self,
        task_type: str,                       # 'stability' or 'fluorescence' (used for logging/artifacts)
        ...
        self.task_type = task_type  # used for naming artifacts/paths
        ...
        self.head = MLPHead(d_latent, 1, head_dropout)
        ...
        # For epoch-wise Spearman on val/test
    ...
    def forward(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        preds = logits.squeeze(-1)                 # [B]
        return preds, batch["labels"].float()
    ...
    def validation_step(self, batch: Dict[str, Any], batch_idx: int):
        ...
        loss = self.loss_fn(preds, targets)
        self.val_loss(loss)
        self._val_preds.append(preds.detach().float().cpu())
        self._val_targets.append(targets.detach().float().cpu())
        
    def test_step(self, batch: Dict[str, Any], batch_idx: int):
        preds, targets = self.forward(batch)
        loss = self.loss_fn(preds, targets)
        self.test_loss(loss)
        self._test_preds.append(preds.detach().float().cpu())
        self._test_targets.append(targets.detach().float().cpu())
        if "protein_id" in batch:
            self._test_pids.extend(list(batch["protein_id"]))

    def _spearman(self, preds: torch.Tensor, targets: torch.Tensor) -> float:
        """Compute Spearman's rho (CPU tensors)."""
        metric = SpearmanCorrCoef().cpu()
        return float(metric(preds, targets).item())

    def on_validation_epoch_end(self):
        preds = torch.cat(self._val_preds)
        targets = torch.cat(self._val_targets)
        rho = self._spearman(preds, targets)
        self.log_dict({
            "val/loss": float(self.val_loss.compute()),
            "val/spearman": float(rho),
        }, prog_bar=True, sync_dist=True)

        self.val_loss.reset(); self._val_preds.clear(); self._val_targets.clear()
    ...
    def on_test_epoch_end(self) -> None:
        ...
        self.log_dict({
            ...
            "test/spearman": float(rho),
        }, prog_bar=True, sync_dist=True)

        # Optional: save raw preds/targets/pids for analysis
        try:
            if getattr(self.trainer, "global_rank", 0) == 0:
                out = {"task": self.task_type, "pids": list(self._test_pids), "preds": preds.cpu(), "targets": targets.cpu()}
                logs_dir = Path("/teamspace/studios/this_studio/PFP_Testing/logs")
                logs_dir.mkdir(parents=True, exist_ok=True)
                torch.save(out, logs_dir / f"{self.task_type}_test_outputs.pt")
        except Exception as e:
            print(f"[Warn] Failed to save test outputs: {e}")
        self.test_loss.reset(); self._test_preds.clear(); self._test_targets.clear(); self._test_pids.clear()