...
# --- CAFA_Style helpers ---------------------------------
def _iter_cafa_label_files(cafa_root: Path, split: str, ontology: str):
    base = Path(cafa_root) / split / ontology
    if not base.exists():
        return
    for pid_dir in base.iterdir():
        if pid_dir.is_dir():
            go_file = pid_dir / "go.txt"
            if go_file.exists():
                yield go_file

def build_go_index_from_dataset(cafa_root: Path, ontology: str, split: str = "train") -> dict:
    """Build {GO_ID -> class_index} by scanning CAFA_Style go.txt files.

    - cafa_root: path to CAFA_Style directory
    - ontology: one of {"bp","mf","cc"}
    - split: typically "train"; determines which labels define the class space
    """
    labels = set()
    for go_file in _iter_cafa_label_files(cafa_root, split, ontology):
        with go_file.open("r", encoding="utf-8") as fh:
            for raw in fh:
                go_id = raw.strip()
                if go_id:
                    labels.add(go_id)
    go_ids = sorted(labels)
    return {go_id: i for i, go_id in enumerate(go_ids)}
