...
class ProteinDataset(Dataset):
    ...
    def __init__(self, data_dir: str, protein_ids: List[str], task_type: str = "mf", split: str = "train", msa_sample_size: Optional[int] = None, *, seed: int = 16):
        """
        ...
        # The same <seed> ensures identical subsampling decisions across *all*
        # runs, devices, workers and epochs, enabling apple-to-apple comparison.
        # We keep it on the instance so every worker gets its own copy.
        self._seed = int(seed)
    ...
    def __getitem__(self, idx):
        ...
        if self.msa_sample_size is not None and len(seqs) > self.msa_sample_size:
            ...
            # Build a reproducible seed unique to this protein but stable across
            # runs.  Hashing guarantees the seed stays in 32-bit range.
            unique_seed = (self._seed + hash(protein_id)) % (2**32)
            rng = random.Random(unique_seed)

            sampled_rest = rng.sample(seqs[1:], k=self.msa_sample_size - 1)
...
class ProteinDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        # Store global seed so we can forward it to every dataset instance
        self.hparams.seed = int(seed)
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            ...
            self.data_train = ProteinDataset(
                ...
                seed=self.hparams.seed, # Pass the seed to the dataset
            )
    ...
    def train_dataloader(self) -> DataLoader[Any]:
        ...
        assert self.data_train is not None, "setup() must be called before train_dataloader"
        return DataLoader(
    ...
    def val_dataloader(self) -> DataLoader[Any]:
        ...
        assert self.data_val is not None, "setup() must be called before val_dataloader"
        return DataLoader(
    ...
    def test_dataloader(self) -> DataLoader[Any]:
        ...
        assert self.data_test is not None, "setup() must be called before test_dataloader"
        return DataLoader(
